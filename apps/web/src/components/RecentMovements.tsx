...
export function RecentMovements() {
  ...
  if (moved.length === 0) {
    return (
      <Card className="p-2">
        <Header />
        <div className="px-4 py-4 text-sm text-ink-dim">Rien n'a bougé cette semaine.</div>
      </Card>
    );
  }
        ...
        children={(m) => {
          ...
          return (
            <>
              ...
              <span
                className={cn("font-mono text-xl font-extrabold tabular-nums", up ? "text-win" : "text-loss")}
              >
                {up ? "+" : ""}
                {m.delta}
              </span>
