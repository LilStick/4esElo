...
export function WrappedPlayer() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["wrapped", d?.year, d?.month, player],
    queryFn: () => getPlayerWrapped(d!.year, d!.month, player),
    enabled: !!d && !!player,
  });
  useTitle(`Wrapped · ${data?.nickname ?? ""}`);

  const label = d ? monthLabel(d.year, d.month) : period;
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      >
        <TbArrowLeft size={16} /> Wrapped du pôle
      </Link>

      {(!d || isError) && (
        <div className="mt-6">
          <EmptyState icon={TbSparkles} title="Wrapped indisponible">
            Impossible de charger ce Wrapped.
          </EmptyState>
        </div>
      )}
      ...
      {d && data && (
        <div className="mt-6 flex flex-col gap-6">
          ...
          {data.matches === 0 ? (
            <EmptyState icon={TbSparkles} title="Aucun match ce mois-ci">
