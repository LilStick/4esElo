...
- **Premier Mode (CS Rating) — V2, après la release v1** (design tranché le 2026-07-15, à ticketer en epic une fois la v1 en ligne — ne pas commencer avant). 2ᵉ source d'ELO = le CS Rating Premier, en **classement + courbe séparés** (échelle 0–35 000, **pas comparable** au ELO Faceit).
  ...
  - **Workflow de dev v2 pendant que la v1 est en prod** (tranché 2026-07-15) :
    - **Branche `v2` longue durée + flag `PREMIER_ENABLED`** (⚠️ amendé 2026-07-18 : le spike Premier a montré un chantier plus gros que prévu — bot Steam GC + download/parse de démos — donc le flag seul ne suffit plus, une branche se justifie). Petites PR sur `v2`. **Règle dure : si `main` reçoit un hotfix/feature prod NON liée à la V2, on rebase `v2` sur `main` immédiatement** — jamais laisser `v2` diverger (sinon enfer de merge au moment de merger la V2). Le flag `PREMIER_ENABLED` reste : quand `v2` mergera sur `main`, le code reste dormant en prod jusqu'au flip.
    - **Coolify auto-deploy depuis `main`** → **merger = déployer**. Donc le code v2 vit **dormant** derrière le flag : `PREMIER_ENABLED=false` en prod → les users ne voient **que la v1**. « Disabled ≠ pas déployé » : le code + les migrations Premier **sont** en prod, juste inertes (routes inutilisées, sync worker sauté = 0 appel Leetify, sélecteur front caché).
    - **Règle dure** : migrations **additives** uniquement (jamais destructif) + code **safe quand le flag est off** → un merge v2 ne casse **jamais** la v1.
    - **Dev** : localhost avec `PREMIER_ENABLED=true` (les deux voient leurs changements). **Staging** (optionnel mais recommandé) = un 2ᵉ déploiement Coolify du **même `main`**, flag ON, **DB séparée**, **URL privée** (basic-auth/allowlist) → répétition générale prod-like pour tester front+back v2 ensemble sans toucher la prod.
    - **Lancement v2** = flipper `PREMIER_ENABLED=true` en prod (1 var d'env + redeploy). Pas de gros merge.
    - **Sécu** : audit `/security-review` de la **v1 avant l'ouverture publique** (front figé) ; puis mini-audit ciblé de la **surface Premier avant de flipper le flag** (clé Leetify, nouvelles routes/deps). Pas de re-audit complet à chaque merge.
  - **Plus tard / si besoin** :
